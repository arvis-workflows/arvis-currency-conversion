...
const currencies = ["AED", "AUD", "BGN", "BRL", "CAD", "CHF", "CNY", "COP", "CZK", "DKK", "EUR", "GBP", "HKD", "HRK", "HUF", "IDR", "ILS", "INR", "ISK", "JPY", "KRW", "MXN", "MYR", "NOK", "NZD", "PHP", "PLN", "RON", "RSD", "RUB", "SEK", "SGD", "THB", "TRY", "TWD", "USD", "ZAR"];
const topCurrencies = ["USD", "EUR", "JPY", "GBP", "CHF", "CAD", "AUD"];
const lastUpdate = new Date(arvish.cache.get('updateDate'));
const nextUpdate = new Date(arvish.cache.get('nextUpdate'));
const l = arvish.cache.get('locale') || 'en-GB';
const locale = l.replace('_', '-');
const baseCurrency = arvish.cache.get('baseCurrency');
const q = arvish.input.toUpperCase().replace('$', 'USD').replace('€', 'EUR').replace('£', 'GBP').replace('¥', 'JPY').split(/(\d*[.,]?\d+)([A-z]{1,3})/);
q.forEach(item => item.split(" ").filter(item => item.length > 0).forEach(item => query.push(item)));

function getOSLocale() {
	return osLocale().then(locale => arvish.cache.set('locale', locale));
}

function cacheRates(data) {
	const rates = data['rates'];
	Object.keys(rates).forEach(cur => arvish.cache.set(cur, rates[cur]));
	...
	arvish.cache.set('nextUpdate', nextUpdateTime);
	arvish.cache.set('updateDate', lastUpdateTime);
	arvish.cache.set('baseCurrency', data['base']);
}

function updateRates(base) {
	return arvish.fetch(`${api}/latest/${base}`).then(cacheRates);
}
...
function addCurrencyListOutput(multiplier, currency) {
	if (currency !== baseCurrency) {
		const rate = arvish.cache.get(currency);
...
if (nextUpdate < new Date()){
	...
	output.push({
		title: `INFO: Rates for ${baseCurrency} updated: ${new Date(arvish.cache.get('updateDate')).toLocaleString(locale)}`,
		subtitle: `next update: ${new Date(arvish.cache.get('nextUpdate')).toLocaleString(locale)}`,
		icon: {
			path: 'settings.png'
		}
	});
